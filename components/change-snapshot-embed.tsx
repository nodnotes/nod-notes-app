...
// Frozen change snapshot as an embedded BoardFlow — host nav/toolbar apply; edits stay local.
...
} from '@/lib/ephemeral-sandbox' // In-memory clone keyed by change id
import {
  CHANGE_PREVIEW_EDIT_MESSAGE,
  fetchBoardChangeSnapshot,
} from '@/lib/board-changes' // Session GET of snapshot JSON + first-edit ping
import { PREVIEW_HOST_TOOLS_MESSAGE } from '@/lib/preview-host-tools' // Draw-tool flag from the host

type ChangeSnapshotEmbedProps = {
  changeId: string // board_changes.id — also the sandbox / preview-frame id
}

/** Lean embed: load snapshot → sandbox at changeId → BoardFlow (embedded, editable, unsaved). */
export function ChangeSnapshotEmbed({ changeId }: ChangeSnapshotEmbedProps) {
  ...
  useEffect(() => {
    ...
    void fetchBoardChangeSnapshot(changeId).then((snapshot) => {
      if (cancelled) return
      if (!snapshot) {
        setFailed(true) // Empty / 404
        return
      }
      const id = mintEphemeralSandboxAt(snapshot, changeId) // Host `[data-page-preview-frame]` matches
      registeredId = id
      if (cancelled) {
  ...
  useEffect(() => {
    ...
    const notify = () => {
      ...
      window.parent.postMessage(
        { type: CHANGE_PREVIEW_EDIT_MESSAGE, pageId: changeId },
  ...
  if (failed) {
    return (
      <div className="flex h-full w-full items-center justify-center text-sm text-gray-500">
        This save has no frozen board to preview.
      </div>
    )
  }
  if (!sandboxId) {
