...
function ReturnToBottomButton({ onClick }: { onClick: () => void }) {
  ...
  useEffect(() => {
    ...
    find()
    const id = window.setInterval(find, 200)
    return () => window.clearInterval(id)
  }, [isChatSidebarOpen])

  if (!isChatSidebarOpen || !slot) return null
...
function StudySetFlowInner({ studySetId }: { studySetId?: string }) {
  ...
  const { setReactFlowInstance, registerSetNodes, isLocked, layoutMode, setLayoutMode, setIsDeterministicMapping, panelWidth: contextPanelWidth, isPromptBoxCentered, lineStyle, setLineStyle, arrowDirection, setArrowDirection, boardRule, boardStyle, setClickedEdge: setContextClickedEdge } = useReactFlowContext()
  ...
  useEffect(() => {
    const checkMinimapPosition = () => {
      ...
      // Gap between default left minimap and prompt approaching from the right
  ...
  useEffect(() => {
    const calculateHoverLeft = () => {
      ...
      // Left-anchored nav toggle center (mirrored from former right-side layout)
      const reactFlowPaddingLeft = parseFloat(getComputedStyle(reactFlowElement).paddingLeft) || 0
      const minimapWidth = 179
      const toggleLeftOffset = 16 // Align with minimap cluster on the left
        ...
        requestAnimationFrame(() => {
          ...
          if (clickedNode) {
            ...
            if (reactFlowElement && promptBox && clickedNode) {
              ...
              // Check if node is a flashcard to determine width
              const isFlashcard = (clickedNode as any).data?.promptMessage?.metadata?.isFlashcard === true
              const panelWidth = isFlashcard ? 600 : 768
  ...
  const deleteNodesByIds = useCallback(async (nodeIdsToDelete: string[]) => {
    if (!studySetId || nodeIdsToDelete.length === 0) return
    ...
    try {
      ...
      // Delete the messages (related records like panel_edges and embeddings will be 
      // automatically deleted due to CASCADE foreign key constraints)
      ...
      if (error) {
        ...
        // Clear the cache and force refetch to ensure deleted flashcards don't reappear
        queryClient.removeQueries({ queryKey: ['flashcards-for-study-set', studySetId] })
        // Invalidate and refetch to get fresh data without deleted flashcards
        await queryClient.invalidateQueries({ queryKey: ['flashcards-for-study-set', studySetId] })
        await queryClient.refetchQueries({ queryKey: ['flashcards-for-study-set', studySetId] })
        return true
      }
    } catch (error) {
  ...
  }, [studySetId, nodes, setNodes, queryClient])
    ...
    <div className="w-full h-full relative">
      ...
      <div
        ...
        style={{
          ...
          // Align with left-side minimap (chat toggle stays on the right)
          left: `${minimapLeft + 16}px`,
        }}
      >
        <div
          className={cn(
