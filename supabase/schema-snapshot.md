...
- **Utility filters**: Layers All/touching/selected and Sets/Captures All boards / This board persist across tab switches (`lib/utility-filter-prefs.ts`).
- **Phone**: pan-tool tap slop + frame finger-up select so taps are not stolen as nav; Add to set drills in on coarse pointers.
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.

## Prior: Top-bar chrome fit
...
- **Calendar connections**: Account Connections manages Notion dates DB, Apple Calendar, and Google Calendar private iCal feeds (`/api/calendar/feed/{token}`); Sets **Schedule** opens the FSRS-6 planner.
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.

## Prior: Smart Draw shapes as frames

- DDL: none. Marker `20260919210149_smart_draw_frames.sql`.
- **Smart Draw shapes**: a stroke that becomes a shape is a frame with that `frameShape` (same fill, border, and chrome as any other frame), not a separate shape node.
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.
...
- **Board menu**: frosted `.tt-menu-surface` wash (not opaque chat grey); portaled to `document.body` with client coordinates so it clears board `isolate` and the welcome overlay.
- **Empty board**: brand and “Click the board…” slide clear of any open menu so the line does not show through the frost.
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.

## Prior: Present mode and capture headers
...
- **Image hover**: board-title glass pill CSS + screen-constant `frameScreenChromeScale(zoom)` (pill + side handles).
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.

## Prior: Image free↔fit contain-fit stable

- DDL: none. Marker `20260918013630_image_free_fit_contain_stable.sql`.
- **Free↔fit contain-fit**: contentFit `data-frame-free-resize` in render (not ProseMirror useEffect); freeze bitmap/`frameBox` on free; remasure live `--tt-image-frame-*` on fit; no max-% jump.
- Schema unchanged; remote DDL top remains `20260914104046_notion_multi_workspace`.

## Prior: Image contain-fit free-frame chrome
...
- **Layout**: board-colored hug plate; desktop center when wider than mode pill; wrap ~1.5–2.5× pill; phone max width = pill width (undo lives inside).
...
## Prior: Notion sync review heal / re-sync
...
- **Layout**: content-sized left cells; zoom % grows with digit count; rotate + pan stay right (`ml-auto`); column **196px**.
- Schema unchanged; remote DDL top remains `20260912192948_panel_edges_canvas_endpoints`.

## Prior: phone utility overlay + keyboard hide
...
- **Phone**: toggle stays right of More (never folded); overlay opens on the map’s right edge (reload starts closed like chat).
- **Chat**: transcript user prompts ChatGPT ice blue (`--nod-chat-user-box`); Ask composer stays chrome grey; prompt↔board threads clip behind the utility bar.
- Schema unchanged; remote DDL top remains `20260912192948_panel_edges_canvas_endpoints`.

## Prior: sidebar open/close presence + prompt border

- DDL: none. Marker `20260913212144_sidebar_open_close_presence_prompt_border.sql`.
- **Open/close**: utility/chat slide with shared presence tween (`useOpenClosePresence`).
- **Prompt chrome**: drop Ask shadow; restore hairline border; align chrome greys.
- Schema unchanged; remote DDL top remains `20260912192948_panel_edges_canvas_endpoints`.

## Prior: utility sidebar Layers / Study / Capture
...
## Prior: frame adjust chrome / rotated hover + Stripe
...
## Prior: homepage ephemeral showcase sandboxes
...
- **Showcase masters** (remote data): Launch Day Deck / Launch Ops Graph / Weekend App Ideas — owner `easayani@goalfish.io`; pastel `fillColor` frames + `boardFont: serif`; env `NEXT_PUBLIC_SHOWCASE_*_BOARD_ID`.
...
## Prior: coming soon access debrand
...
- **Selected tabs**: grey `--nod-selected` wash (not nod-blue) for menus + current board.
- Schema unchanged; remote applied still tops out at `20260811225342`.

## Prior: homepage AI sidebar copy thread stack
...
## Prior: homepage hero icon size

- DDL: none. Marker `20260908223635_homepage_hero_icon_size.sql`.
- **Homepage hero**: brand icon left of **Nod** sized to `0.79cap`.
- Schema unchanged; remote applied still tops out at `20260811225342`.

## Prior: homepage hero lockup thread
...
- Chat↔chat **threads** via `boardLinks.targetTurnId`; multi-selected turns drag together onto the board/composer.
- Thread connect **auto-pan** signs match RF `calcAutoPan` (near left/top → +, near right/bottom → −).
- Schema unchanged; remote applied still tops out at `20260811225342`.

## Prior: Unify AI sparkle light blue and lock-frames icon
...
- Chat↔board thread clip: under-chrome stroke + map tip when the board end is past/behind chat.
- Schema unchanged; remote applied still tops out at `20260811225342`.

## Prior: Chat turn selection persist and link cue scale
